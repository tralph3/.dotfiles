  ...
  (defun setup-font-settings ()
    (let ((fixed-face "Berkeley Mono")
      ...
      (when (and (display-graphic-p)
        ...
        (set-face-font 'default (format "%s-13" fixed-face))
        (set-face-font 'fixed-pitch fixed-face)
        (set-face-font 'variable-pitch variable-face))))
...
#+begin_src emacs-lisp
  ;; TODO: implement moving region as well
  (defun move-line-down-dwim (arg)
    (interactive "p")
    (forward-line 1)
    (transpose-lines arg)
    (forward-line -1))

  (defun move-line-up-dwim (arg)
    (interactive "p")
    (when (> (line-number-at-pos) 1)
        (move-line-down-dwim (* arg -1))))

  (global-set-key (kbd "M-p") #'move-line-up-dwim)
  (global-set-key (kbd "M-n") #'move-line-down-dwim)
#+end_src
...
There must be a bug in [[help:package-vc-install][package-vc-install]], specifically in
[[help:package-read-from-string][package-read-from-string]]. It fails parsing the package string, as
described in [[https://lists.gnu.org/archive/html/help-gnu-emacs/2024-02/msg00053.html][this]] thread. If the function is evaluated before-hand, it
seems to work? It's weird. To not halt the execution of the init file,
I'll just ignore the error for now.
#+begin_src emacs-lisp
  (ignore-errors
...
#+begin_src emacs-lisp
  (use-package etags
    ...
    (when (executable-find "ctags")
      (let ((program "ctags -e -R --fields=+* --extras=+* --kinds-all=* --output-format=etags --languages=all"))
        (setq etags-program-name program)
        (setq etags-regen-program program)))
    :hook (prog-mode . etags-regen-mode))
#+end_src
