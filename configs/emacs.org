...
#+end_src
* Project
General project.el settings.

I don't always want to add a git repository to the projects I make,
but I still want to use emacs features for handling them. This
function lets me add any folder as a project without the need to
litter its root with files.
#+begin_src emacs-lisp
  (defun project-add-local-project (dir)
    "Given a DIR, add it as a project unconditionally."
    (interactive "DDirectory: \n")
    (if (y-or-n-p (format "Add '%s' as a project?" dir))
